...
Sensores, atuadores, IoT e inteligência embarcada

Sensores e atuadores permitem criar formas de interação que vão além de teclado, mouse e toque convencional. Nesta atividade, você irá conceber e prototipar uma solução interativa capaz de perceber uma ação do usuário ou uma condição do ambiente, processar essa informação e produzir uma resposta perceptível.

**Objetivos:**

* Conceber uma solução baseada em uma técnica não convencional de interação;  
* Aplicar sensores, entradas alternativas, atuadores ou formas de feedback na solução proposta;  
* Desenvolver um protótipo funcional ou parcialmente funcional utilizando ao menos uma plataforma de prototipação ou simulação;  
* Representar com clareza o fluxo da interação entre usuário, sistema e ambiente.

**Orientações:**

Escolha um problema, situação ou experiência de seu interesse e desenvolva um protótipo de solução interativa. A proposta deve utilizar uma forma de interação não convencional e demonstrar, de modo claro, como o sistema recebe uma entrada, interpreta essa informação e responde ao usuário ou ao ambiente.

**Fluxo de referência**: Usuário/Ambiente → Sensor ou Entrada → Processamento → Atuador ou Saída → Feedback

**Requisito mínimo**: o protótipo deverá ser desenvolvido com pelo menos uma plataforma de prototipação ou simulação, à livre escolha do estudante.

**Entrega esperada:**

1. **Relatório Técnico Resumido**: descrição do problema, justificativa da solução, sensores/atuadores selecionados, papel do TinyML e evidência do uso da plataforma escolhida.  
2. **Protótipo ou Simulação**: produzido na plataforma escolhida, com registro visual do funcionamento (captura de tela, vídeo curto ou link do projeto).

## **Critérios de avaliação**

| Critério | Peso |
| :---- | :---: |
| Criatividade e relevância do problema real | **20%** |
| Clareza na definição da interação | **15%** |
| Exploração de conceitos de IoT \+ TinyML | **25%** |
| Protótipo funcional na plataforma escolhida (uso efetivo da ferramenta e coerência com o relatório) | **30%** |
| Qualidade da apresentação e documentação | **10%** |

**🔧 Ferramentas Web para Prototipação e Simulação**

Escolha livremente uma das opções abaixo — ou outra plataforma de escopo equivalente, com breve justificativa. O uso de mais de uma ferramenta é permitido, mas não é necessário.

[Edge Impulse Studio](https://edgeimpulse.com) — coleta, anota e treina modelos de TinyML direto no navegador, com datasets prontos e simulação em tempo real, sem exigir hardware inicial. Indicado para visão computacional, áudio e sensores.

[Wokwi](https://wokwi.com) — simulador on-line de Arduino, ESP32 e Raspberry Pi Pico; permite testar sensores e atuadores virtuais integrados a código C++/Arduino. Ótimo para protótipos rápidos de IoT.

[Tinkercad Circuits](https://tinkercad.com/circuits) — ambiente gratuito da Autodesk para montar circuitos e programar Arduino no navegador, sem instalação. Boa porta de entrada em eletrônica, sensores e atuadores.

[Teachable Machine](https://teachablemachine.withgoogle.com) — treinamento rápido de modelos de visão, áudio e pose, sem código, com exportação para TensorFlow Lite. Boa porta de entrada em machine learning.

[MIT App Inventor](https://appinventor.mit.edu) — criação de aplicativos Android com interface visual e extensões de IA; conecta a sensores via Bluetooth/Wi-Fi. Indicado para interações móveis.

**Node-RED** — ferramenta visual para orquestrar fluxos de IoT (sensor → processamento → atuador). O editor roda no navegador, mas normalmente exige uma instância própria; a forma mais rápida de começar sem instalar nada é a versão hospedada gratuita da [FlowFuse](https://flowfuse.com/node-red/), que dá acesso imediato ao editor mediante cadastro.
